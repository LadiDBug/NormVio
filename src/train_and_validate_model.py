...
def preprocess_row_binary(inp):
    ...
    pairs = []
    #parametro non attivo, non entra in questo if
    if use_context and len(context) < min_context:
        return pairs
    
    # mappa ogni categoria agli indici del file. 
    #se c'è una virgola es: "content, format" -> verrà mappato in due categorie
...
def preprocess_data(df, df_rules, cat_idx_mapping, target_class_idx, min_context=2, max_context=4, use_context=False, append_subreddit=None, append_rule=False, is_test=False):
    ...
    # probabilmente coppia TESTO - ETICHETTA
...
def evaluate(iteration, loss, encoder, context_encoder, attack_clf, convid_to_uttr, path_save, best_f1, valid_batch_size, save_model=False):
    ...
    if dev_f1 > best_f1:
        ...
        #TODO: Dovrei iterare per ogni test file e vedere le predizioni su ognuno ? 
    ...
    #range(start_iteration, n_iteration + 1):
        ...
        if iteration > min_iter_to_save:
            if (iteration % validate_every == 0):
                #TODO verificare evaluate per eventualmente togliere f1_test
                best_f1, f1_dev = evaluate(iteration, loss, encoder, context_encoder, attack_clf, convid_to_uttr, path_save, best_f1, valid_batch_size, save_model)
...
#Mettere a True per salvare il modello
...
if args.append_rule:
    ...
    # inizio del ciclo per addestrare un modello separato per ciascusa classe target
    for target_class in target_classes:
        print(f"\ntraining started for {target_class}")
        set_random_seed(args.random_seed)
        if type(target_class) == str:
            target_class = [target_class]
        # mappa il nome della classe alla posizione. 
        target_class_idx = [cat_idx_mapping[c] for c in target_class]

        # crea la cartlla runs dove salverà le metriche 
        writer = SummaryWriter(join("runs/", save_name, ",".join(target_class)))
        path_save_class = join(args.path_save, ",".join(target_class))
        check_or_create_dir(path_save_class)

        # dizionario per dati processati
        processed_data = {}
        # sostituzione: df_rules <--> None
        processed_data['train'] = preprocess_data(data['train'], None, cat_idx_mapping, target_class_idx, args.min_context, args.max_context, args.use_context, append_subreddit=args.append_subreddit, append_rule=args.append_rule, is_test=False)
        processed_data['dev'] = preprocess_data(data['dev'], None, cat_idx_mapping, target_class_idx, args.min_context, args.max_context, args.use_context, append_subreddit=args.append_subreddit, append_rule=args.append_rule, is_test=False)
        # processed_data['test_n_communities_out'] = preprocess_data(data['test_n_communities_out'], None, cat_idx_mapping, target_class_idx, args.min_context, args.max_context, args.use_context, append_subreddit=args.append_subreddit, append_rule=args.append_rule, is_test=True)
        # processed_data['test_n_rules_out'] = preprocess_data(data['test_n_rules_out'], None, cat_idx_mapping, target_class_idx, args.min_context, args.max_context, args.use_context, append_subreddit=args.append_subreddit, append_rule=args.append_rule, is_test=True)
        # processed_data['test_stratified'] = preprocess_data(data['test_stratified'], None, cat_idx_mapping, target_class_idx, args.min_context, args.max_context, args.use_context, append_subreddit=args.append_subreddit, append_rule=args.append_rule, is_test=True)

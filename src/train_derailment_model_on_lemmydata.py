...
def get_convid_to_uttr(split_data):
    ...
    for split_name, data in split_data.items():
        _convid_to_uttr[split_name] = {}
        for d in data:
            _convid_to_uttr[split_name][d['conv_id']] = d['context'] + [d['final_comment']]
...
def evaluate(iteration, loss, encoder, context_encoder, attack_clf, convid_to_uttr, path_save, best_f1, valid_batch_size, save_model=False):
    ...
    for test_name in ['test_n_communities_out', 'test_n_rules_out', 'test_stratified']:
        print(f"Generating predictions for {test_name} set...", end="")
        start = time()
        res_test, df_detection = get_best_val_f1(processed_data[test_name], convid_to_uttr[test_name], encoder, context_encoder, predictor, valid_batch_size)
        test_f1, test_acc = res_test['f1'], res_test['acc']
        print(f" f1: {test_f1*100:.1f}, acc: {test_acc*100:1f}(took {(time()-start)/60:.1f}m)")
        
        # Save test results
        out_file_test = f'{test_name}_detection_{test_f1*100:.1f}_{test_acc*100:.1f}.tsv'
    ...
    # keep track of our best model so far based on dev performance
    if dev_f1 > best_f1:
        print("Validation accuracy better than current best; saving results...")
        best_f1 = dev_f1
        out_file_dev = f'dev_detection_{dev_f1*100:.1f}_{dev_acc*100:.1f}.tsv'
        df_detection.to_csv(join(path_save, out_file_dev), sep='\t')

        # Save config with all test results
        config = {
            'step': iteration,
        ...
        # Add test results to config
        for test_name, results in test_results.items():
            config.update({
                f'{test_name}_f1': results['f1'],
                f'{test_name}_acc': results['acc'],
                f'{test_name}_rec': results['rec'],
                f'{test_name}_prec': results['prec'],
                f'{test_name}_file': results['file']
            })
    ...
    return best_f1, dev_f1, test_results['test_n_communities_out']['f1']  # Return one test f1 for compatibility
...
# Con:
    convid_to_uttr = get_convid_to_uttr({
    'dev': data['dev'],
    'test_n_communities_out': data['test_n_communities_out'],
    'test_n_rules_out': data['test_n_rules_out'],
    'test_stratified': data['test_stratified']
})
    # Initializations
    print('Initializing ...')
    start_iteration = 1
...
def load_dataset(path, filter_removed=False):
    data = {}
    splits = ["train","test_n_communities_out", "test_n_rules_out", "test_stratified","dev"]
    for split in splits:
...
if args.append_rule:
    ...
    with open(path_save_summary,"w") as file:
        file.write("type\tvalid_f1\ttest_n_communities_out_f1\ttest_n_rules_out_f1\ttest_stratified_f1\n")
    ...
    for target_class in target_classes:
        ...
        processed_data = {}
        # sostituzione: df_rules <--> None
        processed_data['train'] = preprocess_data(data['train'], None, cat_idx_mapping, target_class_idx, 
                                        args.min_context, args.max_context, args.use_context, 
                                        append_subreddit=args.append_subreddit, append_rule=args.append_rule, 
                                        is_test=False)
        processed_data['dev'] = preprocess_data(data['dev'], None, cat_idx_mapping, target_class_idx,
                                      args.min_context, args.max_context, args.use_context,
                                      append_subreddit=args.append_subreddit, append_rule=args.append_rule,
                                      is_test=False)

        # Process all test sets
        test_sets = ['test_n_communities_out', 'test_n_rules_out', 'test_stratified']
        for test_name in test_sets:
            processed_data[test_name] = preprocess_data(data[test_name], None, cat_idx_mapping, target_class_idx,
                                             args.min_context, args.max_context, args.use_context,
                                             append_subreddit=args.append_subreddit, append_rule=args.append_rule,
                                             is_test=True)
        ...
        with open(path_save_summary,"a") as file:
            file.write(f"{target_class}\t{res['valid_f1']}\t" +
               f"{res['test_n_communities_out_f1']}\t{res['test_n_rules_out_f1']}\t{res['test_stratified_f1']}\n")
